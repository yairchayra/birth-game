...
const GAMES: {
  ...
  { id: 'who-am-i', title: 'מי אני?',       desc: 'זהי מי מסתתר בתמונה המטושטשת',     emoji: '🔍', path: '/who-am-i', color: 'text-blush-500',    bg: 'from-blush-50 to-rose-50'     },
  { id: 'songs',    title: 'זהי את השיר',   desc: 'מילים מהשיר — תנחשי איזה שיר?',    emoji: '🎵', path: '/songs',    color: 'text-lavender-500', bg: 'from-lavender-50 to-purple-50'},
...
export default function GameSelect() {
  ...
  const handleCardClick = (game: typeof GAMES[0]) => {
    if (confirmReset) return   // don't navigate if confirm dialog is open
    const p = progress[game.id]
    if (p.completed && !p.videoWatched) openVideo(game.id)
              ...
              <motion.div key={game.id} variants={item}>
                ...
                >
                  ...
                  <button
                    ...
                    className={`absolute top-3 right-3 z-10 rounded-full px-2.5 py-1 text-xs font-medium transition-all active:scale-95 ${
                      completed
                        ? 'bg-white/80 hover:bg-white text-gray-600'
                        : 'bg-white/40 hover:bg-white/60 text-gray-400'
                    }`}
                  >
                    ↺ אפס
                  </button>
                  ...
                  <div className="flex items-center gap-4 mt-2">
                    <motion.div className="text-5xl flex-shrink-0" whileTap={{ scale: 0.9, rotate: -5 }}>
                      {completed ? '✅' : game.emoji}
                    </motion.div>
                    <div className="flex-1 min-w-0">
                      <h3 className={`text-xl font-black ${game.color} mb-1`}>{game.title}</h3>
                  ...
                  {confirmReset === game.id && (
                    ...
                    >
                      <div className="mt-1 px-4 py-3 bg-red-50 rounded-2xl flex items-center justify-between border border-red-100">
                        <p className="text-sm text-red-500 font-medium">לאפס את "{game.title}"?</p>
